...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        #     if a_size == 0:
        #         mol_vecs_atoms.append(self.cached_zero_vector)
        #     else:
        ...
        #
        # atoms_v=torch.cat([atoms_v[1:], mol_vecs_atoms], dim=1)
        # r_bonds_v=torch.cat([r_bonds_v[1:], mol_vecs_bonds], dim=1)

        atoms_v=self.batch1(atoms_v[1:])
        r_bonds_v=self.batch1(r_bonds_v[1:])
        # add mol features
        # atoms_v=torch.cat([atoms_v, mol_graph.mol_atoms.cuda()[1:]], dim=1)
        # r_bonds_v=torch.cat([r_bonds_v, mol_graph.mol_bonds.cuda()[1:]], dim=1)

        return atoms_v, r_bonds_v,atom_lr
...
class MPN(nn.Module):
    ...
    def forward(self,
        ...
        atoms_v,r_bonds_v,atom_lr = self.encoder.forward(batch, features_batch)

        return atoms_v,r_bonds_v,t_atoms,t_bonds,atom_lr
