...
class Featurization_parameters:
    ...
    def __init__(self) -> None:
        ...
        self.ATOM_FEATURES = {
            'atomic_num': [0,5,6,7,14,15],#list(range(self.MAX_ATOMIC_NUM)),
            'degree': [0, 1, 2, 3, 4, 5],
...
def get_atom_fdim(overwrite_default_atom: bool = False, is_reaction: bool = False) -> int:
    ...
    """
    k=(not overwrite_default_atom) * PARAMS.ATOM_FDIM + is_reaction * PARAMS.EXTRA_ATOM_FDIM
    k=(not overwrite_default_atom) * PARAMS.ATOM_FDIM + is_reaction * PARAMS.EXTRA_ATOM_FDIM
...
def atom_features(atom: Chem.rdchem.Atom, functional_groups: List[int] = None) -> List[Union[bool, int, float]]:
    ...
    if atom is None:
        ...
        features = onek_encoding_unk(atom.GetAtomicNum() - 1, PARAMS.ATOM_FEATURES['atomic_num']) + \
            ...
            onek_encoding_unk(int(atom.IsInRing()), PARAMS.ATOM_FEATURES['is_ring']) + \
    ...
    * :code:`reaction_mode`:  Reaction mode to construct atom and bond feature vectors
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if not self.is_reaction:
            ...
            atom_lr = [atom.GetAtomicNum() for atom in mol.GetAtoms()]
            self.atom_lr=[]
            for i in range(len(atom_lr)):
                if atom_lr[i]==7 or atom_lr[i]==15 or atom_lr[i]==16:
                    self.atom_lr.append([0,0])
                else:
                    self.atom_lr.append([0,1e6])
            ...
            # Generate BOM list
            BOM_zan=[0]*len(BD_v_batch_one)
            for i in range(len(BD_v_batch_one)):
                BOM_zan[i]=BD_v_batch_one[i].split(',')

            for i in range(self.n_atoms):
                self.t_atoms.append(un_act)

            # Get atoms type
            self.atom_type=[atom.GetAtomicNum() for atom in mol.GetAtoms()]
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    ...
                    self.bond_index+=1
                    self.t_bonds.append(un_act)
                    a1_1=a1+1
                    a2_1=a2+1
                    for i in range(len(BOM_zan)):
                        if (''+str(a1_1)+'') in BOM_zan[i]:
                            if (''+str(a2_1)+'') in BOM_zan[i]:
                                self.t_bonds[self.bond_index]=act
                            elif 'H' in BOM_zan[i]:
                                if self.atom_type[a2]==1:
                                    self.t_bonds[self.bond_index]=act
                                    self.t_atoms[a1]=act
                            elif 'S' in BOM_zan[i] or 'P' in BOM_zan[i] or 'N' in BOM_zan[i]:
                                self.t_atoms[a1]=act
                        elif (''+str(a2_1)+'') in BOM_zan[i]:
                            if (''+str(a1_1)+'') in BOM_zan[i]:
                                self.t_bonds[self.bond_index]=act
                            elif 'H' in BOM_zan[i]:
                                if self.atom_type[a1]==1:
                                    self.t_bonds[self.bond_index]=act
                                    self.t_atoms[a2]=act
                            elif 'S' in BOM_zan[i] or 'P' in BOM_zan[i] or 'N' in BOM_zan[i]:
                                self.t_atoms[a2]=act
            ...
            # Computational molecular features
            des_list = ['MolWt', 'NumHAcceptors', 'NumHDonors', 'MolLogP','TPSA', 'LabuteASA']
            calculator = MoleculeDescriptors.MolecularDescriptorCalculator(des_list)
            self.molecule=list(calculator.CalcDescriptors(mol))

            # Creation connects with atoms and molecules
            self.molecule_atoms=[0]*self.n_atoms
            for i in range(self.n_atoms):
                self.molecule_atoms[i]=self.molecule

            self.molecule_bonds=[0]*(self.bond_index+1)
            for i in range(self.bond_index+1):
                self.molecule_bonds[i]=self.molecule
    ...
    * :code:`a2a`: (Optional): A mapping from an atom index to neighboring atom indices.
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        self.bond_fdim = get_bond_fdim(overwrite_default_bond=self.overwrite_default_bond_features,
                                      ...
                                      is_reaction=self.is_reaction) + self.atom_fdim
        ...
        # All start with zero padding so that indexing with zero padding returns zerosself.t_atoms
        mol_atoms = [[0]*len(mol_graphs[0].molecule_atoms[0])]
        mol_bonds = [[0]*len(mol_graphs[0].molecule_bonds[0])]
        t_atoms = [[0] * 2]
        t_bonds = [[0] * 2]
        ...
        for mol_graph in mol_graphs:
            ...
            t_atoms.extend(mol_graph.t_atoms)
            t_bonds.extend(mol_graph.t_bonds)
            mol_atoms.extend(mol_graph.molecule_atoms)
            mol_bonds.extend(mol_graph.molecule_bonds)
            atom_lr.extend(mol_graph.atom_lr)
            ...
            self.b_scope.append((self.n_bonds, mol_graph.bond_index))
        ...
        self.t_atoms = pd.DataFrame(t_atoms)
        self.t_bonds = pd.DataFrame(t_bonds)
        self.f_atoms = torch.FloatTensor(f_atoms)
        self.f_bonds = torch.FloatTensor(f_bonds)
        self.mol_atoms = torch.FloatTensor(mol_atoms)
        self.mol_bonds = torch.FloatTensor(mol_bonds)
        self.atom_lr = torch.FloatTensor(atom_lr)
        self.atom_lr = self.atom_lr[1:]
    ...
    def get_true(self):
        return self.t_atoms, self.t_bonds
...
def mol2graph(mols: Union[List[str], List[Chem.Mol], List[Tuple[Chem.Mol, Chem.Mol]]],
    ...
    """
    return BatchMolGraph([MolGraph(mol, BD_v_batch_one, af, bf,
                          ...
                          for mol, BD_v_batch_one, af, bf
                          in zip_longest(mols, BD_v_batch, atom_features_batch, bond_features_batch)])
