            ...
            for colsample_bytree_now in colsample_bytree:
                for reg_lambda_now in reg_lambda:
                    for n_estimators_now in n_estimators:
                        for min_child_weight_now in min_child_weight:
                            clf_now=XGBClassifier(objective='binary:logistic',use_label_encoder=False,learning_rate=1e-3,Feval=Feval_xg,tree_method='gpu_hist'
                                                  ,gamma=0.0,min_child_weight=min_child_weight_now,n_estimators=1000,reg_lambda=reg_lambda_now,max_depth=max_depth_now,colsample_bytree=colsample_bytree_now)#,scale_pos_weight=scale_weight
                            ...
                            j_train=sort_xg(clf_now,v,t_xg)

                            if j_val>j_best:# and abs(j_train-j_val)<0.1
                                clf=clf_now
                                j_best=j_val
                                parameter_final=['max_depth_now:',max_depth_now,'colsample_bytree_now:',colsample_bytree_now,'reg_lambda_now:',reg_lambda_now,'n_estimators_now:',n_estimators_now,'min_child_weight_now:',min_child_weight_now]
    ...
    if model_1==False:
        for epoch in range(args.epochs):
            ...
            if epoch==499:
                ...
                model_500 = DiseaseModel(args).cuda()
